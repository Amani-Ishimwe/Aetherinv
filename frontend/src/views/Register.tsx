...
export const Register: React.FC<RegisterProps> = ({ onToggleView, onBackToLanding }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setClientError(null);
    if (!firstname || !lastname || !email || !password || !confirmPassword) { setClientError('Please fill in all fields.'); return; }
    if (!/\S+@\S+\.\S+/.test(email)) { setClientError('Please enter a valid email address.'); return; }
    if (password.length < 6) { setClientError('Password must be at least 6 characters long.'); return; }
    if (password !== confirmPassword) { setClientError('Passwords do not match.'); return; }
    setLoading(true);
    try {
      await register(firstname, lastname, email, password);
    } catch (err: any) {
      setClientError(err.message || 'Registration failed. Email might already exist.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-darkblue-950 flex flex-col items-center justify-center p-4 relative overflow-hidden" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="flex flex-col items-center justify-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Create your business account — it's free</p>
        </div>
        ...
        <div className="bg-darkblue-900/70 backdrop-blur-md rounded-3xl shadow-2xl p-8 border border-darkblue-800/80">
          ...
          {clientError && (
            <div className="mb-5 bg-red-500/10 border border-red-500/30 rounded-2xl p-4 flex items-start gap-3 text-red-400 text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={firstname} onChange={(e) => setFirstname(e.target.value)} placeholder="Jane"
                    className="w-full bg-darkblue-950 border border-darkblue-800 focus:border-brandorange-500 focus:ring-1 focus:ring-brandorange-500/30 rounded-xl py-2.5 pl-9 pr-3 text-slate-200 placeholder-slate-600 transition-all outline-none text-sm" required />
                </div>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com"
                  className="w-full bg-darkblue-950 border border-darkblue-800 focus:border-brandorange-500 focus:ring-1 focus:ring-brandorange-500/30 rounded-xl py-3 pl-11 pr-4 text-slate-200 placeholder-slate-600 transition-all outline-none" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 6 characters"
                  className="w-full bg-darkblue-950 border border-darkblue-800 focus:border-brandorange-500 focus:ring-1 focus:ring-brandorange-500/30 rounded-xl py-3 pl-11 pr-11 text-slate-200 placeholder-slate-600 transition-all outline-none" required />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-500 hover:text-slate-300 transition-colors">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••"
                  className="w-full bg-darkblue-950 border border-darkblue-800 focus:border-brandorange-500 focus:ring-1 focus:ring-brandorange-500/30 rounded-xl py-3 pl-11 pr-4 text-slate-200 placeholder-slate-600 transition-all outline-none" required />
              </div>
            </div>
            ...
            >
              {loading ? (
                ...
                <><i className="fa-solid fa-user-plus"></i> Create My Account</>
              )}
            </button>
          </form>

          <div className="mt-6 text-center border-t border-darkblue-800/80 pt-5">
            <p className="text-slate-400 text-sm">
              Already have an account?{' '}
              <button type="button" onClick={onToggleView} className="text-brandorange-500 hover:text-brandorange-400 font-bold transition-colors focus:outline-none">
                Sign In
              </button>
            </p>
          </div>
